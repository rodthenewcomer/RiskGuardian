...
function buildTrades(rawTxs: RawTx[]): Omit<TradeSession, 'note'>[] {
    ...
    for (let _i = 0; _i < sorted.length; _i++) {
        ...
        if (tx.settledPnl !== null) {
            ...
            // ID is strictly deterministic just like Edge Forensics!
            const deterministicStr = `${tx.baseSymbol}|${createdAt}|${closedAt}|${tx.size}|${Math.round(pnl * 100)}`;
            const hash = cyrb53(deterministicStr);
