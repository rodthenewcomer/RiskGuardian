                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                                        ...
                                        {[...forensics.patterns].sort((a: any, b: any) => Math.abs(b.impact ?? 0) - Math.abs(a.impact ?? 0)).map((p: any, i: number) => {
                                            ...
                                            const prescriptionMap: Record<string, string> = {
                                                ...
                                                'Tilt Cascade': lang === 'fr' ? "3 pertes consécutives = ordinateur fermé pour 4h minimum. Ce n'est pas optionnel. La cascade de tilt est la façon la plus rapide de détruire une semaine rentable. Installez un disjoncteur : 3 pertes → fin de session, aucune exception." : '3 consecutive losses = computer off for minimum 4 hours. This is non-negotiable. Tilt cascade is the fastest way to erase a profitable week. Install a circuit breaker: 3 losses → session ends, no exceptions.',
                                                'Averaging Down': lang === 'fr' ? "Règle absolue : jamais deux trades simultanés sur le même actif dans la même direction. Si un trade est déjà ouvert, attendez qu'il soit fermé avant d'entrer à nouveau. Le scaling est autorisé uniquement avec un plan pre-trade documenté." : 'Absolute rule: never two simultaneous trades on the same asset in the same direction without a documented pre-trade scale plan. If a trade is already open, wait for it to close before re-entering.',
                                                'Account Blow Precursor': lang === 'fr' ? "3 sessions rouges consécutives = pause obligatoire de 48h. Ce n'est pas une suggestion — c'est la règle qui protège votre compte. Pendant cette pause : examinez les 3 sessions, identifiez le déclencheur commun, n'y revenez pas avant d'avoir un plan clair." : '3 consecutive red sessions = mandatory 48-hour pause. This is not a suggestion — it is the rule that protects your account. During the pause: review all 3 sessions, identify the common trigger, do not return until you have a written plan.',
                                                'Sunday Trading': lang === 'fr' ? "N'ouvrez aucune position crypto le dimanche. Sans structure de marché US, les spreads sont plus larges et la liquidité est réduite. Réservez le dimanche à la préparation de la semaine, pas à l'exécution." : 'Open no crypto positions on Sunday. Without US market structure, spreads widen and liquidity thins. Reserve Sunday for weekly preparation, not execution.',
                                            };
